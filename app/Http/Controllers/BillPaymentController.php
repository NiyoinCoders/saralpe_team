...
exit(); */
    ...
    public function paybills(Request $request)
    {
        ...
        $formData->referenceid = $body['referenceid'];
        //$formData->response_code = $res->response_code;
        // $formData->ackno = $res->ackno;
        $formData->operator = $request->operator;
        ...
        $jsonString = json_encode($body['bill_fetch']); // Convert the $res object to a JSON string
        $formData->bill_fetch = $jsonString; // Assign the JSON string to the field
        $formData->save();

        // return redirect()->route('bbps.electricity_bill')->with("status", 'Bill Paid Successfully');
        // if ($res->response_code == 1) {
        ...
        // }
        return response()->json(['success'=>'Bill Paid Successfully!']);
    }
